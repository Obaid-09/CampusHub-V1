...
const uploadResource = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(201, createdResource, "Resource uploaded successfully")
    );
});
...
const getAllResources = asyncHandler(async (req, res) => {
  ...
  const resources = await Resource.find(filter)
    .populate("uploadedBy", "fullname username avatar branch semester")
    .select(
...
const updateResource = asyncHandler(async (req, res) => {
  ...
  if (courseCode !== undefined)
    resource.courseCode = courseCode.trim().toUpperCase();
  ...
  if (thumbnailLocalPath) {
    ...
    resource.thumbnailPublicId = thumbnail.public_id;
  }
  await resource.save({
    validateBeforeSave: false,
  });
